...
try:
    ...
    JinjaSandboxEnv = None  # 有些环境没有 sandbox 模块
...
def load_context_from_excel(
    ...
    key_col: str = "字段名",
    val_col: str = "字段值",
    keep_types: bool = True,#保持原数据类型
    na_as_empty: bool = True,#空值替换为空字符串
) -> Dict[str, Any]:
    ...
    for _, row in df.iterrows():
        key = str(row[key_col]).strip()  # 键名始终转成字符串
        val = row[val_col]
        # 处理缺失值
        if pd.isna(val):
...
# === 新增：读取“给药方案”Sheet 为 list[dict]（没有或为空则返回 []） ===
...
def fill_word_template(
    ...
    **context_kwargs,
    ...
    print(f"✅ Word 模板替换完成：{output_path}")
    return output_path
